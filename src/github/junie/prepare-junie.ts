...
export async function initializeJunieExecution({
    ...
    if (isLinearWorkflowDispatchEvent(context)) {
        try {
            console.log('Linear integration triggered');
            console.log('Linear Payload issueId:', context.payload.issueId);
            console.log('Linear Payload issueIdentifier:', context.payload.issueIdentifier);
            ...
            console.log(`Attempting to add start comment to Linear issue ${context.payload.issueId}`);
            const commentId = await client.addComment(context.payload.issueId, body);
            if (commentId) {
                console.log(`✓ Successfully set Linear init comment ID: ${commentId}`);
                core.setOutput(OUTPUT_VARS.LINEAR_INIT_COMMENT_ID, commentId);
            } else {
                console.warn('⚠️ Linear addComment returned null');
            }
        } catch (linearError) {
            console.error('❌ Error during Linear start comment posting:', linearError);
