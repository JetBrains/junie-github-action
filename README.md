...
- **Conflict Resolution**: Resolve merge conflicts via an `@junie-agent` comment (write "resolve conflicts") or automatic detection
...
Manual conflict resolution trigger:
- Add a PR comment or PR review containing the exact phrase: `resolve conflicts` (for example: `@junie-agent please resolve conflicts`). This will instruct Junie to resolve merge conflicts even when `resolve_conflicts` is not enabled.
